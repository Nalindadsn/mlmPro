...
if ($account) {
	...
	echo 'invalid referral number !';

}else if($refCountl>0 && $_POST['leftORright']=="l"){
	echo 'You cant add to left side !';

}else if($refCountr>0 && $_POST['leftORright']=="r"){
	echo 'You cant add to right side !';

} else {
	// Username doesnt exists, insert new account
	$stmt = $pdo->prepare('INSERT INTO accounts (username, password, email, activation_code,leftORright,user_id) VALUES (?, ?, ?, ?, ?, ?)');
	// We do not want to expose passwords in our database, so hash the password and use password_verify when a user logs in.
	$password = password_hash($_POST['password'], PASSWORD_DEFAULT);
	$uniqid = account_activation ? uniqid() : 'activated';
	$user_id=$_POST['user_id'];
	$leftORright=$_POST['leftORright'];
	$stmt->execute([ $_POST['username'], $password, $_POST['email'], $uniqid,$leftORright, $user_id ]);
